...
use crate::components::{
    CombatStats, Confused, DungeonLevel, Memory, Monster, Position, Viewshed, WantsToMelee,
    WantsToMove, WantsToOpenDoor,
};
...
fn get_move_action(level: &Level, move_idx: i32, step_count: i32) -> WeightedAction {
    let (x, y) = level_utils::idx_xy(&level, move_idx);
    match level.tiles[move_idx as usize] {
        TileType::Door => WeightedAction::new(
            Action::OpenDoor((x, y)),
            reasoner::move_weight(step_count, 2.0),
        ),
        _ => WeightedAction::new(
            Action::MoveTo((x, y)),
            reasoner::move_weight(step_count, 2.0),
        ),
    }
...
fn get_move_action_from_path(
    ...
    match get_next_step(level, start_idx, end_idx) {
        Some((next_step, step_count)) => {
            Some(get_move_action(level, next_step as i32, step_count as i32))
        }
        _ => None,
    }
}
    ...
    fn run(&mut self, data: Self::SystemData) {
            ...
            .join()
        {
            ...
            for (_, enemy_position) in memory.last_known_enemy_positions.iter() {
                ...
                if let Some(action) = get_move_action_from_path(&level, current_idx, idx2) {
            ...
            } else if let Some(destination) = level_utils::get_random_floor_point(&level, &mut rng)
            {
                if let Some(action) =
                    get_move_action_from_path(&level, current_idx, destination as i32)
                {
                    weighted_actions.push(action);
                }
            }
