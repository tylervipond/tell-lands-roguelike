...
fn add_barrel_to_room(room_stamp: &mut Stamp<StampPart<RoomPart>>, rng: &mut RandomNumberGenerator) {
    let mut query_stamp = Stamp::new(vec![
        vec![
            Is(Box::new([Floor, Wall])),
            Is(Box::new([Floor, Barrel])),
            Is(Box::new([Floor])),
        ],
        vec![
            Is(Box::new([Floor, Wall])),
